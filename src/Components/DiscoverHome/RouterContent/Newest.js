...
import "./Popular.css";
import { typeCheck, filteredData, searchList } from "./Popular";
...
const Newest = () => {
  ...
  useEffect(() => {
    fetch(
      "https://api.themoviedb.org/3/movie/now_playing?api_key=3a94078fb34b772a31d9a1348035bed7"
    )
      .then((data) => {
  ...
  useEffect(() => {
    fetch(
      "https://api.themoviedb.org/3/tv/airing_today?api_key=3a94078fb34b772a31d9a1348035bed7"
    )
      .then((data) => {
        if (data.status >= 200 && data.status <= 299) return data.json();
        else throw Error(data.statusText);
